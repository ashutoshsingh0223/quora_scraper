...
conn_no_answer_count = sq.connect('no_answer_count.db')
conn_answer_tag_absent = sq.connect('answer_tag_absent.db')
...
def fetch(driver,dictionary,count,file_name):
	...
	answer_upvote_list = []
	workbook = xlsxwriter.Workbook(f"question_answers.xlsx")
	...
	for key,link in zip(questions,links):
		...
		try:
			...
			try:
				...
				conn_no_answer_count.execute(f"INSERT INTO stocks VALUES ({key},{link})")
				conn_no_answer_count.commit()
				answer_count = 0
				total_views = 0.0
